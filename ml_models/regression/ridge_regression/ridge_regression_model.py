...
# Step-3: Create ridge regression object
ridge_regression_model_object = Ridge(alpha=50, max_iter=100, tol=0.1)
...
ridge_regression_model_object.fit(X_train, y_train)
...
ridge_y_test = ridge_regression_model_object.predict(X_test)
...
ridge_model_accuracy = ridge_regression_model_object.score(X_test, y_test)
print(f"Linear model test accuracy: {linear_model_accuracy} and Ridge Model Test accuracy: {ridge_model_accuracy}")
linear_model_train_accuracy = linear_regression_model_object.score(X_train, y_train)
ridge_model_train_accuracy = ridge_regression_model_object.score(X_train, y_train)
print(f"Linear model train accuracy: {linear_model_train_accuracy} and Ridge Model Train accuracy: {ridge_model_train_accuracy}")
